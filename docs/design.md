...
> 版本: v2.5
> 日期: 2026-05-13
> 状态: 正式设计定稿，已覆盖环境搭建、安装流程、跨平台兼容、MVP 差异说明，可作为实现基线
> 操作主体: OpenClaw Agent
> 目标用户: 个人或 10 人以下小团队
...
backup:
  # MVP：扁平 object key，如 {OSS_PREFIX}/op-xxx.tar.gz；树状 incremental/... 见 §14.1 与 §23
  oss_object: "snapshots/op-20260513-120000-a1b2.tar.gz"
  sha256: "..."
  # 若配置 OSS_ENCRYPTION_KEY 则客户端 AES-256-GCM 加密后上传；未配置则明文 tar.gz
created_at: "2026-05-13T12:00:00+08:00"
...
Phase 4 已落地 SQLite FTS5 + 中文 bigram（见 §19 Phase 4），**不引入** sqlite-vec / LanceDB 等向量库；若 FTS 仍不足，再以真实失败案例评估向量检索。
...
# ./run.sh review show REVIEW_ID --json   # 待实现
./run.sh review answer --id REVIEW_ID --action ACTION --comment "..."
```

`review show` 子命令：**待实现**（MVP 以 `review list --json` 获取任务列表即可）。
...
MVP 中备份与 ingest 解耦：`backup create` 命令独立创建快照并上传 OSS，由每日 cron 触发（见 §12.5）。不配置 OSS 时跳过备份，不在本地生成快照文件。
...
**MVP 说明：** 当前实现的上传对象键多为 `{OSS_PREFIX}/{snapshot_id}.tar.gz` 扁平形式；上图为与 **§14.3 保留策略** 配套的**目标**目录布局，与 MVP 差异见 §23 第 6、7 项。
...
- CLI `__main__` 入口始终输出 JSON（`--json` 为终态可选开关；MVP 经 `run.sh` 调用即可得 JSON，无需依赖显式传 `--json`）。
...
**MVP：** `python -m mini_note.cli`（经 `run.sh` 调用）**始终将业务结果以 JSON 写入 stdout**；子命令上的 `--json` 为预留/兼容，可不传。

**设计终态：** 各子命令显式支持 `--json` / 可选纯文本人类输出，便于脚本与人工混用。
...
| 任务 | 验证 | 状态 |
|---|---|---|
| `install.sh` 脚本 | 一条命令完成 venv 创建、依赖安装、配置初始化 | 完成 |
| `requirements.txt` | 所有依赖可安装，版本兼容 | 完成 |
| 配置模板 | `.env.example` + `meta/config.example.yaml` 可正常复制并启动系统 | 完成 |
| 初始化目录结构 | `init` 后目录完整 | 完成 |
| 定义 schema | source/claim/operation/review 均可校验 | 完成 |
| 单写锁 | 并发 ingest 只有一个成功进入写阶段 | 完成 |
| staging 写入 | 校验失败不影响正式 Wiki | 完成 |
| SQLite schema | 可创建、可迁移、可重建 | 完成 |
| OSS backup | 可上传和校验快照 | 完成 |
| 周期任务骨架 | cron 可触发 `health`、`backup create`、`restore verify`，单写锁正常工作 | 完成 |

### Phase 1: 文本类知识编译 MVP ✅

| 任务 | 验证 | 状态 |
|---|---|---|
| 支持 md/txt/pdf/docx | 每种格式有成功和失败测试 | 完成 |
| claim 提取 | 关键事实有 locator/hash | 完成 |
| source page | 每个 source 有 Wiki 摘要 | 完成 |
| index/overview/log | ingest 后自动更新 | 完成 |
| query | 回答引用 Wiki/claim | 完成 |
| health check | 能发现断链和缺失 claim | 完成 |

### Phase 2: Lint 与审核 ✅

| 任务 | 验证 | 状态 |
|---|---|---|
| claim grounding | claim 找不到来源会报警 | 完成 |
| conflict detection | 冲突 claim 生成 review task | 完成 |
| review answer | 只接受 allowed_actions | 完成 |
| daily health | 输出 JSON 报告 | 完成 |

### Phase 3: 多格式增强 ✅

| 任务 | 验证 | 状态 |
|---|---|---|
| xlsx/pptx | 能生成结构摘要 | 完成 |
| 图片 | 多模态描述带模型证据标记 | 完成 |
| 音视频 | 元数据记录稳定 | 完成 |
| 代码文件 | 结构摘要不执行代码 | 完成 |
...
已实现 SQLite FTS5 + 中文 bigram 全文检索，支持 scope 过滤和 rank 排序。后续可根据真实查询案例继续优化。
...
本节记录当前 MVP 实现与上文「正式设计」之间的 **10 项已知差异**，避免长期误解。

| # | 差异项 | 设计文档 / 终态目标 | MVP 实现 | 原因/计划 |
|---:|---|---|---|---|
| 1 | backup 格式 | `tar` + zstd（`.tar.zst`）+ 客户端加密为推荐组合 | `tar.gz` + 可选 **AES-256-GCM**（`OSS_ENCRYPTION_KEY`）；未配置密钥则上传未加密包 | zstd 未纳入默认依赖链；gzip 易兼容。加密见 §14.1、§7.5 |
| 2 | `--json` 行为 | 各命令显式支持 `--json` | **入口始终输出 JSON** 到 stdout；子命令 `--json` 多为预留 | OpenClaw/脚本集成优先；见 §15.1、§15.3 |
| 3 | `query --owner` | 流程中有 owner 解析、CLI 曾示例 `--owner` | **无 `--owner`**，仅 `--scope`（及 `--question` 等） | 小团队默认 `shared` + `owner_id` 元数据即可；按 owner 过滤查询留待后续 |
| 4 | `review show` | `./run.sh review show REVIEW_ID --json` | **未实现**（§13.2 已标注） | 以 `review list --json` 满足 MVP；单条 `show` 为便利性补充 |
| 5 | claim `quote_hash` | 关键 claim 绑定 `quote_hash` 等证据字段 | CLI 启发式 claim 常 **`quote_hash` 为空**，定位符较粗 | 强证据链由 Skill 侧 LLM 补全；CLI 侧重摄入与占位 |
| 6 | OSS 对象键与路径 | 树状 `snapshots/incremental/YYYY/MM/DD/...` 等 | 扁平 `{OSS_PREFIX}/{snapshot_id}.tar.gz`（ingest/backup 各自约定） | 先跑通上传下载与校验；分层路径可与生命周期规则一并升级 |
| 7 | OSS / 本地快照 **保留策略** | §14.3 多级 TTL（增量/日/周/月） | **本地 staging**：有 OSS 时 `backup create` 保留最近 7 个本地 fallback；**未**在应用内实现 OSS 侧 TTL | 与 bucket 生命周期规则解耦；MVP 控本地磁盘与 `backup_log.jsonl` |
| 8 | 备份触发时机 | 每次 ingest 触发增量快照 | 仅 `backup create` 命令触发，由每日 cron 调用 | 减少个人使用的冗余快照；容忍丢失最多一天变更 |
| 9 | 本地模式备份 | 无 OSS 配置时创建本地快照并保留 | 无 OSS 配置时 `backup create` 直接返回 `skipped=true`，不创建任何快照文件 | 本地快照无容灾价值 |
| 10 | **向量检索** | Phase 4 曾列 sqlite-vec、LanceDB 等为候选 | Phase 4 已用 **FTS5 + bigram**，**未**引入向量库 | 嵌入式、零向量依赖；若 FTS 不足再评估 §19 表中向量方案 |
...
| v2.5 | 2026-05-13 | 新增 §23「MVP 与正式设计差异」8 项；§7.5/§14.1 统一为 tar.gz + 可选 AES-256-GCM；§8.2 与 Phase 4 对齐；§13.2 `review show` 待实现；§15.1 query 无 `--owner`、`review answer --id`、MVP 恒 JSON；§15.3 区分 MVP 与终态 `--json`；§19 Phase 0–4 均标 ✅ |
