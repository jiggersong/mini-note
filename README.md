...
# 导入后清理 inbox 副本（移至 processed/），不修改来源目录
./import.sh ~/Documents/notes --cleanup

# 摄入单个文件
./run.sh ingest --file doc.xlsx --owner user-default --scope shared

# 扫描 inbox 目录批量摄入（含进度追踪 + 时间预估）
./run.sh ingest --scan-inbox --owner user-default --scope shared

# 摄入后清理 inbox
./run.sh ingest --scan-inbox --cleanup processed
...
# Lint 检查（按严重级别过滤）
./run.sh lint --changed-only --min-severity warning
...
./import.sh ~/Documents/notes --cleanup          # 导入后清理 inbox 副本
./import.sh ~/Documents/notes --force            # 跳过磁盘预检
```

脚本流程：扫描目录 → 磁盘空间预检 → 时间预估（stderr JSON Lines）→ 复制到 `raw/inbox/users/` → 批量摄入（含进度追踪，每 30s/里程碑输出）→ 索引重建 → 健康检查。`--cleanup` 将已处理文件移至 `raw/inbox/processed/YYYYMMDD/`。
...
| 命令 | 说明 |
|------|------|
| `./import.sh DIR` | 批量导入已有目录（含预检 + 进度） |
| `./import.sh DIR --cleanup` | 批量导入，处理后清理 inbox |
| `init --workspace PATH` | 初始化知识库目录 |
| `ingest --file PATH` | 摄入文件，生成 Wiki 页面和 claim |
| `ingest --scan-inbox` | 批量摄入 inbox 目录（含进度追踪） |
| `ingest --scan-inbox --cleanup processed` | 批量摄入，处理后移至 processed/ |
| `query --question "..."` | 检索 Wiki 页面和 claim 素材 |
| `health --json` | 健康检查报告 |
| `lint --changed-only` | 增量 Lint 检查 |
| `lint --min-severity warning` | Lint 按严重级别过滤 |
